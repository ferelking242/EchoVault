package com.aivos.echovault.data.repository
  ...
  class ClipboardRepository @Inject constructor(
      ...
      fun getEntriesByRange(start: Long, end: Long): Flow<List<ClipboardEntryEntity>> = clipboardDao.getEntriesByDateRange(start, end)
      ...
      suspend fun addEntry(content: String): Long {
          ...
          return clipboardDao.insertEntry(ClipboardEntryEntity(content = content, contentType = detectContentType(content)))
      }
      ...
      private fun detectContentType(content: String): ContentType {
          ...
          return when {
              trimmed.matches(Regex("^\d{4,8}$")) -> ContentType.OTP
              trimmed.matches(Regex("^[+]?\d[\d\s\-().]{6,}$")) -> ContentType.PHONE
              trimmed.contains("@") && trimmed.contains(".") -> ContentType.EMAIL
              trimmed.startsWith("http://") || trimmed.startsWith("https://") || trimmed.startsWith("www.") -> ContentType.URL
              trimmed.contains("\n") || trimmed.lines().size > 3 -> ContentType.MULTILINE
              trimmed.contains("{") || trimmed.contains("fun ") || trimmed.contains("def ") || trimmed.contains("class ") -> ContentType.CODE
